...
def run(url):
    ...
    soup = download(rooturl,headers)    
    urls_tmp = soup.find_all("a", class_="item-top item-1")
    for item in urls_tmp:                                                    ########遍历soup.find_all结果
        ops=re.match(r'.+{}/(.+)/.+html">'.format(match_rooturl(rooturl)),str(item)).group(1)   ########从网址中匹配出title,作为目录名
        if ops in lst:                                                       #######查重
            continue                                                         #######查重
        lst.append(ops)                                                      #######查重
        ...
        soup_second_url = download("http:{}".format(item.get("href")),headers)
        ...
        for item1 in urls_tmp1:                                               ########遍历soup.find_all结果
            ...
            soup_third_url = download(url_short,headers)
